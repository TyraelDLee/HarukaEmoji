...
async function initialize(reload) {
    ...
    await chrome.storage.local.set({
        ...
        'availableBranch': "https://gitee.com/tyrael-lee/HarukaEmoji/releases",
        'downloadFileName': '',
        "video_id_list": [],
        "pgc_id_list": [],
        "article_id_list": [],
        'unreadData': '{"at":0,"chat":0,"like":0,"reply":0,"sys_msg":0,"up":0}',
        'unreadMessage': 0/*'{"biz_msg_follow_unread":0,"biz_msg_unfollow_unread":0,"dustbin_push_msg":0,"dustbin_unread":0,"follow_unread":0,"unfollow_push_msg":0,"unfollow_unread":0}'*/,
        'dynamicList': [],
        'notificationList': '',
        'videoInit': true,
        'dynamicInit': true,
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        if (request.msg === "get_LoginInfo") {
            chrome.cookies.get({url: 'https://www.bilibili.com/', name: 'bili_jct'},
                function (jct) {
                    (jct === null) ? chrome.storage.local.set({'jct': -1}, () => {
                    }) : chrome.storage.local.set({'jct': jct.value}, () => {
                    });
        ...
        if (request.msg === "get_LIVE_BUVID") {
            chrome.cookies.get({url: 'https://live.bilibili.com/', name: 'LIVE_BUVID'}, function (cookie) {
                (cookie === null) ? chrome.storage.local.set({'buvid': -1}, () => {
                }) : chrome.storage.local.set({'buvid': cookie.value}, () => {
                });
            ...
            chrome.cookies.get({url: 'https://www.bilibili.com/', name: 'DedeUserID'}, (uid) => {
                if (uid === null || uid === undefined)
                    ...
                    fetch(`https://api.bilibili.com/x/space/acc/info?mid=${uid.value}`, {
                        ...
                        .then(res => res.json())
                        .then(result => {
                            result["code"] === 0 && result["data"] !== null ? sendResponse({
                                login: true,
                                vip: result["data"]['vip']['type']
                            }) : sendResponse({login: true, vip: 1});
                        }).catch(e => {
                            console.log(`Error occurs @anonymous function chrome.runtime.onMessage.addListener-get_LoginStatus, failed to fetch (dest: api.bilibili.com/x/space/acc/info); ${e}`);
                        sendResponse({login: true, vip: 1});
                    });
                }
            });
...
function queryLivingRoom(uids, blackListNT, blackListHB) {
    ...
    chrome.storage.local.get(['uuid', 'notificationList', 'watchingList', 'heartRhythm', 'buvid', 'medalList', 'jct'], (info) => {
        ...
        fetch("https://api.live.bilibili.com/room/v1/Room/get_status_info_by_uids?requestFrom=rua5", {
                    ...
                    if (typeof data !== 'undefined') {
                        chrome.notifications.getAll((n) => {
                            ...
                            chrome.storage.local.set({
                                'notificationList': newNotificationList.stringify()/*, 'watchingList': watchingList*/,
                                'heartRhythm': liveInfo
                            }, () => {
                            });
            ...
            .catch(msg => {
                errorHandler('getFollowing', msg, 'queryLivingRoom()');
            });
    });
}
                ...
                chrome.storage.local.get(['windowSize'], (win) => {
                    if (win[0] === 0 || win[1] === 0)
                        ...
                        chrome.windows.create({
                            url: `https://${nid.split(":")[2]}${nid.split(":")[1]}`,
                            width: win[0],
                            height: win[1]
                        });
                });
...
function reloadCookies() {
    chrome.cookies.get({url: 'https://www.bilibili.com/', name: 'DedeUserID'},
        function (uid) {
            uid === null || uid.expirationDate === null || uid.expirationDate < Date.parse(new Date()) / 1000 ? chrome.storage.local.set({'uuid': -1}, () => {
            }) : chrome.storage.local.set({'uuid': uid.value}, () => {
            });
                ...
                if ((uids.uuid === -1) && uids.uuid !== uids.p_uuid) {
                    ...
                    chrome.alarms.getAll((alarms) => {
                        for (let name of alarms) {
                            ...
                            if (name.name !== 'checkUpd' && name.name !== 'getUID_CSRF') chrome.alarms.clear(name.name).then(r => {
                                console.log(`${name.name} was cleared ${r}`)
                            });
                        }
                    })
                }
    ...
    chrome.cookies.get({url: 'https://live.bilibili.com/', name: 'LIVE_BUVID'},
        function (buvid) {
            (buvid === null) ? chrome.storage.local.set({'buvid': -1}, () => {
            }) : chrome.storage.local.set({'buvid': buvid.value}, () => {
            })
        });
}
    ...
    chrome.storage.sync.get(["checkIn"], (result) => {
        if (result.checkIn) {
            fetch("https://api.live.bilibili.com/xlive/web-ucenter/v1/sign/DoSign", {
                ...
                errorHandler('checkIn', msg, 'checkingIn()');
            });
        }
    });
}
...
function errorHandler(handler, msg, at) {
    console.log(`Error occurs @function ${at}; ${msg}`);
    let minutes = 0.2;
...
function exchangeBCoin(JCT) {
    ...
    fetch("https://api.bilibili.com/x/vip/privilege/receive?requestFrom=rua5", {
        ...
        errorHandler('bcoin', msg, 'exchangeBCoin()');
    });
}
    ...
    chrome.storage.sync.get(["bcoin"], (result) => {
        if (result.bcoin) {
            fetch("https://api.bilibili.com/x/vip/privilege/my", {
                ...
                errorHandler('bcoin', msg, 'queryBcoin()');
            });
        }
    });
}
                    ...
                    chrome.storage.sync.get(["dynamicPush", "blackListVideo", "videoPush", "pgcPush", "articlePush"], (result) => {
                        if (json['code'] !== 0) reject(json['code']);
                            ...
                            if (result.dynamicPush) {
                                ...
                                // console.log(dynamic_id_list)
                                dynamic_id_list = null; // GC
                                resolve(200);
                            }
                        }
                    });
...
async function videoNotify(UUID) {
    try {
        ...
        console.log(e);
        errorHandler('getNewVideos', e, 'videoNotify()');
    }
...
function dynamicNotify() {
    ...
    chrome.storage.local.get(['dynamicList', 'dynamicInit'], (r) => {
        ...
        fetch(`https://api.vc.bilibili.com/dynamic_svr/v1/dynamic_svr/dynamic_new?type_list=1,2,4`, {
            ...
            .catch(e => {
                errorHandler('getNewDynamics', e, 'dynamicNotify()')
            });
    });
}
...
function daka(dakaUid, jct, dkMsg) {
    if (dakaUid.length === 0) {
        ...
        fetch("https://api.live.bilibili.com/live_user/v1/Master/info?uid=" + dakaUid[0], {
            ...
            .catch(e => {
                console.log(`Error occurs @function daka(); ${e}`);
            });
    }
}
...
function findVideo(vid) {
    return fetch("https://api.bilibili.com/x/web-interface/view?" + vid, {
        ...
        .catch(e=>{
            console.log(`Error occurs @function findVideo(); ${e}`);
        });
}
...
function checkUpdate(url) {
    ...
    fetch(url + new Date().getTime(), {
        ...
        .catch(e => {
            console.log(`Error occurs @function checkUpdate; ${e}`);
            if (!url.includes('github')){
                console.log(`Unable to access Gitee, retry at GitHub source.`);
                checkUpdate("https://tyraeldlee.github.io/HarukaEmoji/?_=");
            }else{
                console.log(`Unable to access Gitee and GitHub, stop check today.`);
            }
...
function QRSComplex(rhythm, jct) {
    ...
    return fetch(`https://live-trace.bilibili.com/xlive/data-interface/v1/x25Kn/${rhythm['id'][2] === 0 ? 'E' : 'X'}`, {
        ...
        .catch(e => {
            console.log(`Error occurs @function QRSComplex; ${e}`);
        });
...
function refreshHeartBeatList() {
    chrome.storage.local.get(['uuid'], (info) => {
        fetch(`https://api.live.bilibili.com/xlive/web-ucenter/user/MedalWall?target_id=${info.uuid}`, {
            ...
            console.log(`Error occurs @function refreshHeartBeatList(); ${e}`);
            chrome.alarms.create('refreshHBRetry', {delayInMinutes: 1});
        });
    });
}
