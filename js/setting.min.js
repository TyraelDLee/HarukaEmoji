!function(){function a(d,e,f,g=!1,h=!1,i=""){return fetch(h?`https://api.bilibili.com/x/relation/followings/search?vmid=${e}&pn=${d}&ps=${k}&order=desc&order_type=attention&name=${i}`:`https://api.bilibili.com/x/relation/${g?`whispers?`:`followings?vmid=${e}&`}pn=${d}&ps=${k}`,{method:"GET",credentials:"include",body:null}).then(a=>a.json()).then(h=>{if(0===h.code){for(let a=0;a<h.data.list.length;a++)s.push(h.data.list[a].mid),c(h.data.list[a].face,h.data.list[a].uname,h.data.list[a].mid,h.data.list[a].official_verify.type,h.data.list[a].official_verify.desc,j.get(h.data.list[a].mid));return d===f&&(g?(document.getElementById("loading").style.display="none",b()):a(1,e,1,!0).then(a=>{a<=k&&(document.getElementById("loading").style.display="none")})),r=!0,"undefined"==typeof h.data.total?h.data.list.length+1:h.data.total}return 0})}function b(){fetch(`https://api.live.bilibili.com/xlive/web-ucenter/user/MedalWall?target_id=${i}`,{method:"GET",credentials:"include",body:null}).then(a=>a.json()).then(a=>{if(0===a.code)for(const b of a.data.list)-1===s.indexOf(b.medal_info.target_id)&&c(b.target_icon,b.target_name,b.medal_info.target_id,b.official-1,"",[b.medal_info.level,b.medal_info.medal_color_border,b.medal_info.medal_color_start,b.medal_info.medal_color_end,b.medal_info.medal_name],!0)}).catch(a=>{})}function c(a,b,c,e,f,g=[0,0,0,0,0],i=!1){c-=0;let j=0===e?"background-position: var(--personal-verify);":1===e?"background-position: var(--organization-verify);":"display: none;",k=document.createElement("div");k.classList.add("rua-setting-user-block"),k.innerHTML=`<div class="rua-setting-user-info">
            <a href="https://space.bilibili.com/${c}" target="_blank" class="user-face"><img src="${a}"><span class="verified" style="${j}"></span></a>
            <div class="user-name"><div style="display: flex"><a href="https://space.bilibili.com/${c}" target="_blank">${b}</a><span class="medal" ${-1===g.indexOf(0)?"":`style="display:none;"`}><div class="medal-border" style="border-color: rgb(${d(g[1])})"><div class="medal-label" style="background-image: linear-gradient(45deg, rgb(${d(g[2])}), rgb(${d(g[3])}));"><span class="medal-name">${g[4]}</span></div><div class="medal-level" style="color: rgb(${d(g[1])})">${g[0]}</div></div></span></div><span class="verify-text">${f}</span></div>
        </div>
        <div class="rua-setting-button-group">
            <div class="button-host">
                <section class="button">
                    <div class="checkbox">
                        <input id="rua-input-hb-${c}" type="checkbox" ${-1===p.indexOf(c)&&-1===g.indexOf(0)?"checked":""} ${-1===g.indexOf(0)?"":"disabled"}>
            ...
            <div class="button-host">
                <section class="button">
                    <div class="checkbox">
                        <input id="rua-input-dk-${c}" type="checkbox" ${-1===o.indexOf(c)&&-1===g.indexOf(0)?"checked":""} ${-1===g.indexOf(0)?"":"disabled"}>
                        <label ${-1===g.indexOf(0)?"":`class="btn-disabled"`}></label>
                    </div>
                </section>
            </div>
            <div class="button-host">
                <section class="button">
                    <div class="checkbox">
                        <input id="rua-input-live-${c}" type="checkbox" ${-1!==l.indexOf(c)||i?"":"checked"} ${i?"disabled":""}>
                        <label ${i?`class="btn-disabled"`:""}></label>
                    </div>
                </section>
            </div>
            <div class="button-host">
                <section class="button">
                    <div class="checkbox">
                        <input id="rua-input-video-${c}" type="checkbox" ${-1!==m.indexOf(c)||i?"":"checked"} ${i?"disabled":""}>
                        <label ${i?`class="btn-disabled"`:""}></label>
                    </div>
                </section>
            </div>
