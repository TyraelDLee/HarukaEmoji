...
# HarukaEmoji
## bilibili近日修改了api导致开播通知与直播挂机功能暂不可用，正在想办法修复。其他功能目前不受影响。现在的源码换回了之前较慢的api，且关注数太多时可能会表现不正常，但只能先凑合用了；；请直接下载源码更新。
...
## 版本记录
### ver 5.24
- 禁用了Page Visibility事件监听，在多个页面切换不会再造成视频卡顿、重载

### ver 5.23
- 添加了开播自动点赞点亮牌子的相关功能。自动打卡现已默认关闭，之前开启的也可以在设置中关闭。
- 修复了设置页显示不正常的问题。
- 修复了某些情况下放映室列表显示无法正常显示的问题。
